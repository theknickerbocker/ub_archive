...
int send_packets(char *ip, int sleeptime, FILE *file, FILE *file2);
...
int main(int argc, char *argv[]){
	...
	send_packets(argv[1], sleeptime, fp, fp2);
}
 ...
 */
int send_packets(char *ip, int sleep_rate, FILE *file, FILE *file2)
{
    int sockfd;
	...
	while(total_time < 10)
	{
		...
		fflush(stdout);
		// gettimeofday(&current, NULL);
		// long long trans_dur = ((long long)current.tv_sec) * ((long long)1000000) + current.tv_usec - cur_time;
		// int sleep_dur = sleep_rate - trans_dur;
		// if ( sleep_dur > 0)
		// {
		// 	usleep(sleep_dur);
		// }
